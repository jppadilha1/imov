...
- [ ] **4.4 Use Case de Sync**
  - [ ] 🔴 Criar `application/use-cases/sync/SyncProspectosUseCase.spec.ts` (Valida fluxo: db local pendentes -> envia nuvem -> atualiza entities resolvendo endereço -> salva ids novos local)
  - [ ] 🟢 Implementar `SyncProspectosUseCase.ts`
...
- [x] **6.1 Hooks e Contexto de Auth**
  - [ ] 🔴 Criar `presentation/hooks/useAuth.spec.ts` (Testar usando `renderHook` e Providers mock)
  - [x] 🟢 Implementar `AuthContext.tsx`
  - [ ] 🟢 Implementar `useAuth.ts`


- [ ] **6.2 Tela de Login**
  - [ ] 🔴 Criar `app/(auth)/login.spec.tsx` (Testando inputs e acionamento de clicks usando React Native Testing Library)
  - [ ] 🟢 Implementar `app/(auth)/login.tsx` (Layout flexbox + Stylesheet)

- [ ] **6.3 Tela de Registro**
  - [ ] 🔴 Criar `app/(auth)/register.spec.tsx`
  - [ ] 🟢 Implementar `app/(auth)/register.tsx`

- [ ] **6.4 Auth Guard**
  - [ ] 🔴 Criar `app/_layout.spec.tsx` (Valida proteção de rotas com base no check auth)
  - [ ] 🟢 Implementar `app/_layout.tsx` redirecionando usuários não autenticados
...
- [ ] **7.1 Hook de Captura**
  - [ ] 🔴 Criar `presentation/hooks/useCapture.spec.ts`
  - [ ] 🟢 Implementar `useCapture.ts` gerenciando loadings e erros da View

- [ ] **7.2 Tela de Captura e Componentes**
  - [ ] 🔴 Criar `presentation/components/OfflineBanner.spec.tsx` e implementar logo depois.
  - [ ] 🔴 Criar `app/(tabs)/capture.spec.tsx`
  - [ ] 🟢 Implementar `app/(tabs)/capture.tsx` utilizando Use Case via Container
...
- [ ] **8.1 Hook de Prospectos**
  - [ ] 🔴 Criar `presentation/hooks/useProspectos.spec.ts`
  - [ ] 🟢 Implementar `useProspectos.ts` com pull-to-refresh embutido no state

- [ ] **8.2 Componentes e Telas de Lista**
  - [ ] 🔴 Criar `presentation/components/ProspectoCard.spec.tsx` e `SyncBadge.spec.tsx` implementando logo em seguida
  - [ ] 🔴 Criar `app/(tabs)/list/index.spec.tsx` (Lista FlatList com os hooks)
  - [ ] 🟢 Implementar `app/(tabs)/list/index.tsx`

- [ ] **8.3 Tela de Detalhe**
  - [ ] 🔴 Criar `presentation/components/StatusSelector.spec.tsx` testando chamadas onChange e implementar.
  - [ ] 🔴 Criar `app/(tabs)/list/[id].spec.tsx`
  - [ ] 🟢 Implementar `app/(tabs)/list/[id].tsx`
...
- [ ] **9.1 Tela do Mapa (Home)**
  - [ ] 🔴 Criar `presentation/components/MapMarkerCallout.spec.tsx`
  - [ ] 🟢 Implementar callouts com navegação embutida
  - [ ] 🔴 Criar `app/(tabs)/map.spec.tsx` (Validar lógica de clusterização mocada)
  - [ ] 🟢 Implementar `app/(tabs)/map.tsx` com a biblioteca MapView e Clustering
...
- [ ] **11.1 Hook de Sync Real**
  - [ ] 🔴 Criar `presentation/hooks/useSync.spec.ts` garantindo os disparos de sync via `NetInfo` change handler
  - [ ] 🟢 Implementar `useSync.ts` e integrar na _layout/tab bar para sincronização invisível
  - [ ] 🟢 Adicionar toast views confirmando sync ou notificando erros
